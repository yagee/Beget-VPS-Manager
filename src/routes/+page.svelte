		...
		<section class="dashboard" transition:fade={{ duration: 180 }}>
			<header class="masthead">
				...
				<div class="masthead-actions">
					...
					<button class="outline" onclick={handleLogout} type="button">Logout</button>
				</div>
			</header>
			...
			<div class="toolbar">
				...
				<div class="toolbar-box wide">
					<span>Last sync</span>
					<strong>{dashboard ? formatTimestamp(dashboard.summary.fetchedAt) : 'Waiting for data'}</strong>
				</div>
			</div>
			...
			{:else if dashboard}
				<div class="grid">
					{#each filteredServers as server (`${server.id}:${server.currentCpuCount}:${server.currentMemory}:${server.status}`)}
						<VpsCard onRefresh={loadDashboard} {server} token={begetToken ?? ""} />
					{:else}
						<div class="empty-state">
							<h3>No VPS cards match the current filter.</h3>
							<p>Change the search query or disable the reconfigurable-only toggle.</p>
						</div>
					{/each}
				</div>
			{/if}
...
<style>
	...
	.shell {
		...
		overflow: hidden;
		background:
			radial-gradient(circle at top left, rgba(24, 145, 163, 0.22), transparent 25%),
			radial-gradient(circle at top right, rgba(255, 184, 75, 0.13), transparent 23%),
	...
	.kicker {
		margin: 0 0 0.75rem;
		font: 700 0.78rem/1 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
	...
	button {
		...
		font: 700 0.84rem/1 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
		cursor: pointer;
	}
