...
let priceDeltaMonth = $derived(projectedPriceMonth - server.currentPriceMonth);
...
<article class="card">
	...
	<div class="resources">
		...
		<div class="control price-card">
			...
			<div class="price-row" class:gain={priceDeltaMonth < 0} class:loss={priceDeltaMonth > 0}>
				<span>Delta</span>
	...
	{#if server.flags.archived || server.flags.migrating || server.flags.rescueMode || server.flags.hostUnavailable}
		<div class="flags">
			{#if server.flags.archived}<span>archived</span>{/if}
			{#if server.flags.migrating}<span>migrating</span>{/if}
			{#if server.flags.rescueMode}<span>rescue</span>{/if}
			{#if server.flags.hostUnavailable}<span>host issue</span>{/if}
		</div>
	{/if}

	<footer class="actions">
		<button class="ghost" disabled={!isDirty || saving} onclick={resetDraft} type="button">
			Reset
		</button>
		<button
			class="primary"
...
<style>
	.card {
		...
		background:
			...
			radial-gradient(circle at top left, rgba(255, 198, 90, 0.14), transparent 34%);
	...
	.name {
		font: 700 1.12rem/1.1 'Space Grotesk', 'Avenir Next', 'Segoe UI', sans-serif;
		color: #f4f7fb;
	}
	...
	.meta {
		...
		flex-wrap: wrap;
		gap: 0.4rem 0.8rem;
		font: 0.79rem/1.35 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
	...
	.status {
		...
		border-radius: 999px;
		font: 700 0.72rem/1 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
	...
	.metrics-label {
		margin: 0;
		font: 700 0.76rem/1 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
	...
	.period-button.active {
		background: linear-gradient(135deg, rgba(248, 184, 75, 0.22), rgba(125, 231, 243, 0.18));
		color: #f9fbfd;
	...
	button {
		...
		border-radius: 999px;
		padding: 0.8rem 1rem;
		font: 700 0.85rem/1 'IBM Plex Mono', 'SFMono-Regular', Consolas, monospace;
		cursor: pointer;
	}
